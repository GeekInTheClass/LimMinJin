...
let recent = hallOfFame.filter(){ $0.year > 2000 }


//: sorted 함수를 이용해 최근부터 오래된 순으로 기록을 나열한 latest 배열을 만들어라.
let latest = hallOfFame.sorted(by: {$0.year > $1.year})
//: reduce 함수를 이용해 전체 평균 totalAverage 값을 구하라.
let totalAverage = hallOfFame.reduce(0, {$0 + $1.average}) / Double(hallOfFame.count)
